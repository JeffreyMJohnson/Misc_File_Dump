...
void Circle::ApplyCollision(Circle* a_pOther)
{
	...
	//collision normal
	float deltaY = a_pOther->m_vVelocity.y - a_pOther->GetPosition().y;
	float deltaX = a_pOther->m_vVelocity.x - a_pOther->GetPosition().x;
	Vector3 N = Vector3(-deltaY, deltaX, 0);

	//normalize it
	N = N.Normalize();

	//magnitude of the projection
	Vector3 P = N * -(this->m_vVelocity.Dot(N));
	
	Vector3 thisNewVelocity = P * 2.0f + this->m_vVelocity;

	//compute for other
	deltaY = this->m_vVelocity.y - this->GetPosition().y;
	deltaX = this->m_vVelocity.x - this->GetPosition().x;

	N = Vector3(-deltaY, deltaX, 0);
	N = N.Normalize();

	P = N * -(a_pOther->m_vVelocity.Dot(N));

	a_pOther->m_vVelocity = P * 2.0f + a_pOther->m_vVelocity;
	this->m_vVelocity = thisNewVelocity;
